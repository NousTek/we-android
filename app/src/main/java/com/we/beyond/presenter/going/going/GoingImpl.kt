...
/** It implements going api */
class GoingImpl (going : GoingPresenter.IGoingView) :
    GoingPresenter.IGoingPresenter
{
    ...
    override fun onGoing(context: Context, jsonObject: JsonObject)
    {
        try {
    ...
    /** It calls the Going api which takes below parameter
     */
    private fun getDataToPost(context: Context, jsonObject: JsonObject)
    {
        try {
            ...
            val response: Single<GoingPojo> = goingApi.Going(jsonObject)
            response.subscribeOn(Schedulers.newThread()).observeOn(AndroidSchedulers.mainThread())
                .subscribe(object : DisposableSingleObserver<GoingPojo>() {
                    override fun onSuccess(goingPojo: GoingPojo) {
