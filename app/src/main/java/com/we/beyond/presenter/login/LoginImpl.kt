...
import com.we.beyond.model.ExternalUserInfo
...
class LoginImpl(login: LoginPresenter.ILoginView) : LoginPresenter.ILoginPresenter {
    ...
    override fun onFbLogin(context: Context, jsonObject: JsonObject) {
        try {
            if (ConstantMethods.checkForInternetConnection(context)) {
                getFBDataToPost(context, jsonObject)
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
    ...
    override fun onGoogleLogin(context: Context, jsonObject: JsonObject) {
        try {
            if (ConstantMethods.checkForInternetConnection(context)) {
                getGoogleDataToPost(context, jsonObject)
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
    ...
    private fun getFBDataToPost(context: Context, jsonObject: JsonObject) {
        try {
            val loginApi = retrofit.create(LoginApi::class.java)
            val response: Single<LoginPojo> = loginApi.loginWithFacebook(jsonObject)
            response.subscribeOn(Schedulers.newThread()).observeOn(AndroidSchedulers.mainThread())
                .subscribe(object : DisposableSingleObserver<LoginPojo>() {
                    override fun onSuccess(loginPojo: LoginPojo) {
    ...
    private fun getGoogleDataToPost(context: Context, jsonObject: JsonObject) {
        try {
            val loginApi = retrofit.create(LoginApi::class.java)
            val response: Single<LoginPojo> = loginApi.loginWithGoogle(jsonObject)
            response.subscribeOn(Schedulers.newThread()).observeOn(AndroidSchedulers.mainThread())
                .subscribe(object : DisposableSingleObserver<LoginPojo>() {
                    override fun onSuccess(loginPojo: LoginPojo) {
