...
class ConnectCategoriesAdapter(
    val context : Context,
    connectCategories: ArrayList<ConnectCategories>
): RecyclerView.Adapter<ConnectCategoriesAdapter.ViewHolder>()
{
    ...
    override fun onCreateViewHolder(
        ...
        val view = LayoutInflater.from(context).inflate(R.layout.list_connect_categories,parent,false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int)
    {
        ...
        /** It selects and un select view and call resetValue and getDataToPost function of ConnectIssueActivity*/
        holder.itemView.setOnClickListener {

            setSelected(position)

            (context as ConnectIssueActivity).connectCategoryId = category!![position]._id.trim()
            (context as ConnectIssueActivity).resetValue()
            (context as ConnectIssueActivity).getDataToPost()
    ...
    private fun setSelected(position: Int)
    {
        for(i in 0 until viewArray.size) {
            viewArray[i].txt_connect_category.setTextColor(ContextCompat.getColor(context, R.color.colorPrimary))
