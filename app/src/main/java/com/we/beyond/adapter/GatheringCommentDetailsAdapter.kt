...
/** It binds gathering comment details data set to views */

class GatheringCommentDetailsAdapter
    (
    val context: Context,
    commentDetails: ArrayList<CommentsReplyDetails>,
    commentId: CommentDetailsPojo
) : RecyclerView.Adapter<GatheringCommentDetailsAdapter.ViewHolder>() {
        ...
        try {
            ...
            holder.edit.setOnClickListener {
                if(replyDetails!=null) {
                    ...
                    holder.moreLayout.visibility = View.GONE
                    holder.moreLayout.startAnimation(AnimationUtils.loadAnimation(context, R.anim.slide_out_down))
                    (context as GatheringCommentActivity).commentsAdapter!!.notifyDataSetChanged()


                    if(replyDetails!![position].tags !=null && replyDetails!![position].tags.isNotEmpty())
                    {
                        (context as GatheringCommentActivity).tagsList!!.clear()
                        (context as GatheringCommentActivity).tagUserName!!.clear()

                        for(i in 0 until replyDetails!![position].tags.size)
                        {
                            (context as GatheringCommentActivity).tagsList!!.add(replyDetails!![position].tags[i])
                            (context as GatheringCommentActivity).tagUserName!!.add(replyDetails!![position].tagNames[i])

                            println("tags from server ${ (context as GatheringCommentActivity).tagsList}")
                            println("tags names from server ${ (context as GatheringCommentActivity).tagUserName}")
                        }
            ...
            if (replyDetails!![position]!= null) {
                ...
                holder.reply.setOnClickListener {
                    (context as GatheringCommentActivity).commentId = commentId!!.data._id
                    (context as GatheringCommentActivity).commentPosition = position
                    (context as GatheringCommentActivity).getData()
                }
