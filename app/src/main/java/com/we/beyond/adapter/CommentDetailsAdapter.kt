...
/** It binds comment details data set to views */

class CommentDetailsAdapter(
    val context: Context,
    commentDetails: ArrayList<CommentsReplyDetails>,
    commentId: CommentDetailsPojo
) : RecyclerView.Adapter<CommentDetailsAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        try {
            ...
            /** It open more layout with animation */
            ...
            holder.edit.setOnClickListener {
                 if(replyDetails!=null) {
                        ...
                        holder.moreLayout.visibility = View.GONE
                        holder.moreLayout.startAnimation(AnimationUtils.loadAnimation(context,R.anim.slide_out_down))
                        (context as CommentActivity).commentsAdapter!!.notifyDataSetChanged()


                     if(replyDetails!![position].tags !=null && replyDetails!![position].tags.isNotEmpty())
                     {
                         (context as CommentActivity).tagsList!!.clear()
                         (context as CommentActivity).tagUserName!!.clear()

                         for(i in 0 until replyDetails!![position].tags.size)
                         {
                             (context as CommentActivity).tagsList!!.add(replyDetails!![position].tags[i])
                             (context as CommentActivity).tagUserName!!.add(replyDetails!![position].tagNames[i])

                             println("tags from server ${ (context as CommentActivity).tagsList}")
                             println("tags names from server ${ (context as CommentActivity).tagUserName}")
                         }
...
//                                sweetAlertDialog.show()
            ...
            if (replyDetails!![position]!= null) {
                ...
                holder.reply.setOnClickListener {
                    (context as CommentActivity).commentId = commentId!!.data._id
                    (context as CommentActivity).commentPosition = position
                    //(context as NearByIssueDetailsActivity).resetValue()
                    (context as CommentActivity).getData()
                }
